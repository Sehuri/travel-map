...
(async () => {
  ...
  try {
    ...
    assert.equal(await page.locator('.journey-card').count(), 5);
    ...
    assert.match(journeyCardsText, /2025 齐鲁与东北冰雪之旅.*济南 → 烟台 → 大连 → 哈尔滨 → 沈阳/s);
    assert.match(journeyCardsText, /2025 江南新春之旅.*无锡 → 上海/s);
