...
test('homepage offers a global basemap alongside AMap China details', () => {
  ...
  assert.match(page, /data-view="world"[^>]*aria-pressed="false"/);
  assert.match(page, /data-view="china"[^>]*aria-pressed="true"[^>]*>中国细节/);
  assert.match(page, /id="world-map"[^>]*hidden/);
  assert.match(app, /let mapView = "china"/);
  assert.match(app, /setMapView\("china"\)/);
