...
- 默认“中国细节”视图使用高德地图 JS API 2.0，展示中国行政区填色、到访城市光点与周边国家概貌。
- “全球”视图使用 Leaflet + CARTO，展示中国与更远的海外足迹；高德加载失败时会自动切换到此视图。
- `assets/data.js` 中的坐标仍保留通用 WGS-84；只有中国大陆坐标在进入高德地图前由 `assets/map-engine.js` 转换为 GCJ-02，不污染全球地图坐标。
- 高德 JS API Key 会出现在浏览器请求中，这是 JS API 的正常用法；`securityJsCode` 只保存在 Supabase，不进入 GitHub。
- 如果高德配置缺失、超时或报错，仍可切换“全球”视图浏览足迹。
...
1. 全球视图能显示中国和日本的全部足迹，切换“仍在期待”后也能显示海外愿望地点。
2. 中国细节视图显示高德署名、国内行政区填色和城市光点。
3. 南京、北京、三亚等标记位置无明显偏移。
4. 点击标记仍会打开城市详情并更新 `?city=` 链接。
5. 临时填错 Key 或停止代理时，中国细节视图显示错误提示，全球视图不受影响。
